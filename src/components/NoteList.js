...
const NoteList = ({ notes, username, handleAddNote, accessToken }) => {
  ...
  const handleUpdateNote = () => {
    try {
      console.log('Id notatki:', editedNote.id);
      fetch(`http://localhost:8080/api/v1/notes/${editedNote.id}`, {
        method: 'PUT',
        headers: {
        ...
        .then((data) => {
          const updatedNotes = notesList.map((note) =>
            note.id === editedNote.id ? { ...note, title: data.title, content: data.content } : note
          );
      ...
      console.error('Błąd podczas przekształcania _id do ObjectId:', error);
    }
  };
  ...
  const handleDeleteNote = (note) => {
    setDeletedNote(note);
    console.log(note.id);
    setSelectedDeleteNoteId(note.id);
    setSelectedNoteId(null);
  };

  const handleConfirmDelete = () => {
    fetch(`http://localhost:8080/api/v1/notes/${deletedNote.id}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
      ...
      .then((res) => {
        ...
        // No need to parse JSON for a DELETE request
        return res.text(); // Or use res.json() if the server returns a meaningful response
      })
      .then(() => {
        const updatedNotes = notesList.filter((note) => note.id !== deletedNote.id);
        setNotesList(updatedNotes);
        setDeletedNote(null);
        setSelectedDeleteNoteId(null);
      })
        ...
        <Accordion defaultActiveKey="0" className="d-inline-block w-100">
          {notesList.map((note, index) => {
            if (note.username === username) {
              return (
                <Accordion.Item eventKey={index.toString()} key={`${note.id}-${index}`}>
